...
export default function App() {
  ...
  const downloadPoster = useCallback(async () => {
    ...
    try {
      ...
      link.download = `Pallas-English-Poster-${Date.now()}.png`;
  ...
  return (
    <div className="min-h-screen bg-rose-50/30 text-[#1A1A1A] font-sans selection:bg-brand-red/10 relative overflow-hidden">
      {/* Decorative Background Icons */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden opacity-[0.08] z-0">
        <Star className="absolute top-10 left-10 text-red-400" size={120} />
        <Sparkles className="absolute top-1/4 right-20 text-brand-red" size={100} />
        <GraduationCap className="absolute bottom-20 left-1/4 text-brand-red" size={150} />
        <Trophy className="absolute bottom-1/3 right-10 text-red-500" size={130} />
        <ImageIcon className="absolute top-1/2 left-10 text-red-400" size={80} />
        <Mic className="absolute top-20 right-1/3 text-red-500" size={110} />
        <Star className="absolute bottom-10 right-1/4 text-red-400" size={90} />
      </div>
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-8 sm:py-12">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 bg-slate-100/50 p-2 rounded-2xl border border-slate-200">
            {([
              ...
              <button key={mode} onClick={() => setContentMode(mode)}
                className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base
                  ${contentMode === mode ? 'bg-gradient-to-r from-red-600 to-brand-red text-white shadow-lg' : 'text-slate-600 hover:bg-slate-200'}`}
              >
                {icon} {label}
              </button>
            ))}
            ...
            >
              ...
              {lessonHistory.lessons.length > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-brand-red text-white text-[9px] font-black rounded-full flex items-center justify-center shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10">
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl border-4 border-red-100 overflow-hidden min-h-[400px] sm:min-h-[600px] flex flex-col relative">
              {/* Preview Header */}
              <div className="p-3 sm:p-5 border-b-4 border-red-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-rose-50/30">
                  ...
                  {readingText && (
                    <button onClick={() => setShowTranslation(!showTranslation)}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm
                        ${showTranslation ? 'bg-brand-red text-white' : 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-50'}`}
                    >
                  ...
                  {readingText && (
                    <button onClick={downloadPoster} disabled={isDownloading}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold text-white transition-all shadow-lg
                        ${isDownloading ? 'bg-red-400 cursor-not-allowed' : 'bg-brand-red hover:bg-red-800 shadow-red-100'}`}
                    >
                ...
                <AnimatePresence mode="wait">
                  {isGenerating ? (
                    ...
                    <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full flex flex-col items-center gap-4">
                      ...
                      {/* AI Prompt Debug */}
                      {generatedPrompt && (
                        <div className="w-full max-w-[600px] p-4 bg-rose-50/50 rounded-xl border border-red-100/50">
                          <p className="text-[10px] uppercase font-bold text-red-400 mb-1 tracking-widest">AI Prompt</p>
                          <p className="text-xs text-red-900/70 italic leading-relaxed">{generatedPrompt}</p>
                        </div>
                      )}
                    </motion.div>
                  ) : (
