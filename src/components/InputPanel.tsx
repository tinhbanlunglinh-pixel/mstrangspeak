...
export const InputPanel: React.FC<InputPanelProps> = (props) => {
  ...
  return (
    <div className="lg:col-span-4 space-y-6">
      <section className="bg-white p-4 sm:p-6 rounded-[2rem] shadow-xl border-4 border-red-100 space-y-6 relative overflow-hidden">
        <div className="absolute -top-10 -right-10 w-32 h-32 bg-rose-50 rounded-full opacity-40 blur-2xl" />
        <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-rose-50 rounded-full opacity-40 blur-2xl" />
        
        <div className="relative z-10 space-y-6">
          <div>
            <label className="flex items-center gap-2 text-sm font-black text-brand-red mb-3 uppercase tracking-wider">
              {contentMode === "generate" && <><Type size={18} className="text-brand-red" /> Chủ đề hoặc Từ vựng</>}
              {contentMode === "image" && <><ImageIcon size={18} className="text-brand-red" /> Tải ảnh lên</>}
              {contentMode === "useInput" && <><FileText size={18} className="text-brand-red" /> Văn bản bài đọc</>}
            </label>
            ...
            {imagePreview && contentMode !== "image" && (
              <div className="mt-3 flex items-center gap-2 p-2 bg-red-100/50 rounded-xl border border-red-200">
                ...
                <span className="text-[10px] font-black text-red-800 uppercase truncate">Hình ảnh đã sẵn sàng</span>
                <button onClick={() => setImagePreview(null)} className="ml-auto p-1.5 text-red-600 hover:text-red-500 hover:bg-white rounded-lg transition-all">
                  <X size={14} />
                </button>
              </div>
            )}
            {contentMode === "useInput" && (
              <div className="mt-2 flex justify-end">
                <button onClick={() => docFileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-1.5 bg-red-100 text-red-800 rounded-lg text-[10px] font-black hover:bg-red-200 transition-colors uppercase tracking-wider">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-black text-brand-red mb-3 uppercase tracking-wider">
              <GraduationCap size={18} className="text-red-400" /> Trình độ Tiếng Anh
            </label>
            <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
              {LEVELS.map((lvl) => (
                <button key={lvl} onClick={() => setLevel(lvl)}
                  className={`px-1 py-2 rounded-xl text-[10px] font-black border-2 transition-all
                    ${level === lvl ? 'bg-brand-red border-brand-red-dark text-white shadow-[0_4px_0_#5c0a0a] -translate-y-1' : 'bg-white border-slate-200 text-slate-900 hover:border-red-300 hover:bg-rose-50'}`}
                >{lvl}</button>
              ))}
            </div>
          </div>
          ...
          {error && (
            <div className="p-4 bg-red-50 border-2 border-red-100 rounded-2xl space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button onClick={onRetry} className="flex items-center gap-1.5 px-3 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-xl text-xs font-black transition-colors">
                  🔄 Thử lại
                </button>
              </div>
            </div>
...
const ImageUploadArea: React.FC<any> = ({ imagePreview, isDragging, imageInputRef, handleImageUpload, handleDragOver, handleDragLeave, handleDrop }) => (
  <div onClick={() => imageInputRef.current?.click()} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
    className={`relative w-full aspect-video rounded-2xl border-4 border-dashed transition-all flex flex-col items-center justify-center gap-3 cursor-pointer overflow-hidden
      ${isDragging ? 'border-brand-red bg-rose-50/50' : 'border-red-100 bg-rose-50/30 hover:border-red-300 hover:bg-rose-50'}`}
  >
    {imagePreview ? (
      <>
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
          <div className="bg-white p-3 rounded-full text-brand-red shadow-xl"><RefreshCw size={24} /></div>
        </div>
      </>
    ) : (
      <>
        <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-red-400 shadow-sm border border-red-100"><Upload size={32} /></div>
        <div className="text-center">
          <p className="font-black text-red-800 uppercase text-sm tracking-wide">Nhấn để chọn ảnh</p>
          <p className="text-[10px] text-red-600 font-bold uppercase mt-1">Hoặc kéo thả ảnh vào đây</p>
        </div>
      </>
    )}
...
const TextInputArea: React.FC<any> = ({ topic, setTopic, contentMode, isDragging, isProcessingFile, handleDragOver, handleDragLeave, handleDrop, handlePaste }) => (
  <div className={`relative transition-all duration-200 ${isDragging ? 'scale-[1.02]' : ''}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
    <textarea value={topic} onChange={(e) => setTopic(e.target.value)} onPaste={handlePaste}
      ...
      className={`w-full h-36 sm:h-40 p-4 bg-rose-50/30 border-2 rounded-2xl focus:ring-4 focus:ring-brand-red/20 focus:border-brand-red transition-all resize-none text-slate-900 placeholder:text-slate-400 font-semibold text-sm sm:text-base
        ${isDragging ? 'border-brand-red bg-rose-50' : 'border-slate-100'}`}
    />
    {isProcessingFile && (
      <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] rounded-2xl flex items-center justify-center gap-2 text-red-800 font-bold animate-pulse">
        <RefreshCw className="animate-spin" size={16} /> Đang xử lý file...
      </div>
    )}
    {isDragging && (
      <div className="absolute inset-0 border-4 border-dashed border-red-400 bg-rose-50/30 rounded-2xl flex flex-col items-center justify-center gap-2 pointer-events-none">
        <Upload size={32} className="text-red-600 animate-bounce" />
        <span className="font-black text-red-700 uppercase">Thả file vào đây</span>
      </div>
    )}
  </div>
);
