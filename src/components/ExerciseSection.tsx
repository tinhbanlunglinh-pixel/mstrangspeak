...
export const ExerciseSection: React.FC<ExerciseSectionProps> = ({ exerciseData, onComplete, savedScore }) => {
  ...
  const renderQuestion = (q: ExerciseQuestion, index: number) => {
    ...
    const getLabelClass = (key: string) => {
      ...
      if (userAnswer === key) {
        base += ' border-brand-red bg-rose-50/50 shadow-sm';
      } else {
    ...
    return (
      <div key={q.id} className="p-4 sm:p-5 bg-white rounded-xl border border-slate-100 shadow-sm mb-4">
        ...
        <div className="flex gap-3">
          <span className="font-black text-red-600 mt-0.5">{index + 1}.</span>
                  ...
                  {(orderingIndices[q.id] || []).length === 0 ? (
                    ...
                    (orderingIndices[q.id] || []).map((idx, itemIndex) => (
                      <motion.button
                        ...
                        className="px-3 py-1.5 bg-red-600 hover:bg-red-700 disabled:bg-red-600 text-white rounded-lg text-xs sm:text-sm font-bold shadow-sm flex items-center gap-1 transition-all"
                ...
                {Object.entries((q as MultipleChoiceQuestion | TranslationQuestion | ErrorCorrectionQuestion | FillBlankQuestion).options).map(([key, val]) => (
                  <label key={key} className={getLabelClass(key)}>
                    <input 
                      ...
                      className="w-4 h-4 text-brand-red border-slate-300 focus:ring-brand-red shrink-0" 
                    />
  ...
  return (
    <div className="w-full max-w-[800px] mx-auto mt-8 space-y-6">
      <div className="bg-gradient-to-br from-red-700 to-red-800 p-6 sm:p-8 rounded-[2rem] shadow-xl text-white text-center">
        ...
        {submitted && score !== null && (
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-6 bg-white/20 p-4 rounded-2xl border border-white/30 backdrop-blur-sm inline-block">
            <div className="flex items-center gap-3 justify-center mb-1">
              ...
              <span className="text-sm font-black uppercase tracking-widest text-red-100">Điểm số của bạn</span>
            </div>
      ...
      <div className="space-y-8 bg-white/50 p-4 sm:p-6 rounded-[2rem] border-2 border-red-100 shadow-sm">
        {allQuestions.map((section) => {
          if (!section.questions || section.questions.length === 0) return null;
          return (
            <div key={section.type} className="space-y-4">
              <h3 className="text-lg font-black text-red-800 border-b-2 border-red-200 pb-2">{section.title}</h3>
      ...
      {!submitted && (
        <div className="sticky bottom-6 flex justify-center z-10">
          <button
            ...
            className="px-8 py-4 bg-brand-red text-white rounded-full font-black text-lg shadow-2xl shadow-red-500/50 hover:-translate-y-1 hover:shadow-red-500/60 transition-all flex items-center gap-3"
          >
